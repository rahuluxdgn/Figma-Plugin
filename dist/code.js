...
function getSourceNodes() {
    ...
    if (!selection.length)
        throw new Error("Select at least one frame, component, instance, or section.");
    if (selection.some(node => !["FRAME", "COMPONENT", "INSTANCE", "SECTION"].includes(node.type)))
        throw new Error("Every selected node must be a frame, component, instance, or section.");
    return selection;
}
function applyMode(node, collectionId, modeId) {
    ...
    if ("children" in node)
        for (const child of node.children)
            applyMode(child, collectionId, modeId);
}
